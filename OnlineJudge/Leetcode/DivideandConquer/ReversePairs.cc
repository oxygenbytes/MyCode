...
private:
    int mergeSort(vector<int>& nums,int lo, int hi){
        if(hi <= lo) return 0; // 此时不存在满足条件的逆序对
        int mid = (hi - lo) / 2 + lo;
        int res = 0;
    ...
    int merge(vector<int>& nums, int lo,int mid, int hi){
        int cnt = 0;
        vector<int> f(hi-lo+1,0);
        int i = lo,j = mid+1;
        // count reverse pair
        while(i <= mid && j <= hi){
              // lo...mid and mid...hi already been sorted
